...
├── story-list.tsx
├── story-card.tsx
├── paragraph-card.tsx (턴별 좋아요 포함)
├── author-profile-popover.tsx
├── writing-editor.tsx
├── writing-guide.tsx (AI 가이드)
├── challenge-card.tsx
├── award-winners-gallery.tsx
└── navbar.tsx
```
...
| Route | Method | 설명 |
| --- | --- | --- |
| `/api/guide` | POST | AI 작성 가이드 생성 |
| `/api/summary` | POST | 스토리 줄거리 요약 (Phase 2) |

### Supabase RPC Functions

- `get_story_authors_count(room_id)`: 스토리별 작가 수 반환
- `check_and_acquire_lock(room_id, user_id)`: Lock 획득 시도
- `release_lock(room_id, user_id)`: Lock 해제
...
1. **완성 작품 갤러리** (`/completed`)
   - 완성된 스토리만 모아서 보는 전용 페이지
   - 커버 이미지, 총 턴 수, 참여 작가 수, 좋아요 수 표시
   - 장르/인기순/최신순 필터링
   - 페이지네이션 (9개씩)

2. **스토리 생성 페이지** (`/story/create`)
   - 사용자가 직접 새 스토리 방 생성
   - 제목, 장르, 첫 문단, 태그 입력
   - 실시간 유효성 검사

3. **작가 프로필 상세 페이지** (`/profile/[id]`)
   - 작가별 통계 및 배지
   - 시작한 스토리 / 참여한 스토리 탭

4. **인기 작가 섹션**
   - 홈 페이지에 인기 작가 검색 및 카드
   - 작가별 통계 (스토리 수, 좋아요, 턴 수)
...
6. **작가 프로필 팝오버**
   - 문단 카드의 작가명 클릭 시 미니 프로필 표시
   - Glassmorphism 디자인

7. **AI 작성 가이드**
   - 스토리 상세 페이지에서 톤 분석 및 작성 팁 제공
   - 문장 예시 제안
