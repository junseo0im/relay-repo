...
import { completedStories } from "@/lib/sample-data"
import { GENRE_COLORS } from "@/lib/types"
import { BookCheck, Heart, Users, FileText, Search, SlidersHorizontal, Calendar, ChevronLeft, ChevronRight } from "lucide-react"
...
export default function CompletedStoriesPage() {
  ...
  const filteredAndSortedStories = useMemo(() => {
    ...
    if (genre !== "all") {
      ...
      const genreKorean = genreMap[genre]
      if (genreKorean) {
    ...
    // Search filter
    if (search) {
      const searchLower = search.toLowerCase().replace(/#/g, "").trim()
    ...
    switch (sort) {
      case "likes":
        ...
        break
      case "turns":
        filtered.sort((a, b) => b.totalTurns - a.totalTurns)
        break
      case "authors":
        filtered.sort((a, b) => b.totalAuthors - a.totalAuthors)
        break
      case "latest":
      default:
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-16">
          ...
          <div className="flex items-center gap-6 mt-6">
            <div className="flex items-center gap-2 text-sm text-muted-foreground">
              ...
              <span>{completedStories.length}개의 완성 작품</span>
            </div>
            <div className="flex items-center gap-2 text-sm text-muted-foreground">
              <Heart className="h-4 w-4" />
              <span>{completedStories.reduce((sum, s) => sum + s.totalLikes, 0).toLocaleString()} 좋아요</span>
            </div>
          </div>
        </div>
      </section>
                  ...
                  <div className="group relative bg-card rounded-2xl border border-border/50 shadow-sm hover:shadow-xl hover:shadow-primary/10 hover:-translate-y-1 transition-all duration-300 overflow-hidden h-full">
                    ...
                    {story.coverImage && (
                      <div className="relative h-48 overflow-hidden">
                        ...
                        <Badge className={`absolute top-3 left-3 ${GENRE_COLORS[story.genre] || GENRE_COLORS["자유"]}`}>
                          {story.genre}
                        </Badge>
                      </div>
                    )}
                    ...
                    <div className="p-5">
                      ...
                      <div className="flex items-center gap-1 text-xs text-muted-foreground mt-3">
                        ...
                        <span>{new Date(story.completedDate).toLocaleDateString('ko-KR')}</span>
                      </div>
                    </div>
                  </div>
                </Link>
