      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {winnerStories.map((story, index) => {
          ...
          return (
            ...
            >
              {/* Animated Background */}
              <div className={`absolute inset-0 ${config.bgColor} opacity-50 group-hover:opacity-70 transition-opacity duration-500`} />
              
              {/* Rank Badge */}
              <div className="absolute top-4 right-4 z-10">
                <div className={`flex items-center gap-1.5 px-3 py-2 rounded-xl ${config.bgColor} border-2 ${config.borderColor} backdrop-blur-sm shadow-lg`}>
              ...
              <div className="relative p-6 pt-16">
                {/* Trophy Icon */}
                <div className={`inline-flex p-4 rounded-2xl ${config.bgColor} border ${config.borderColor} mb-4 group-hover:scale-110 transition-transform duration-300`}>
                ...
                <Link href={`/story/${story.id}`}>
                  <Button 
                    className="w-full bg-gradient-to-r from-primary to-secondary hover:opacity-90 shadow-lg group-hover:shadow-xl transition-all"
                  >
                    작품 읽기
                  </Button>
                </Link>
