...
export default function ProfileSettingsPage() {
  ...
  const [preferredGenres, setPreferredGenres] = useState<string[]>(["판타지", "로맨스"])
  ...
  useEffect(() => {
    ...
    if (stored) {
      setDisplayName(stored.displayName ?? "사용자")
      setAvatarUrl(stored.avatarUrl ?? "")
      setBio(stored.bio ?? "")
      setPreferredGenres(stored.preferredGenres ?? ["판타지", "로맨스"])
    }
  }, [])
  ...
  const handleSave = async () => {
    ...
    const payload = { displayName, avatarUrl, bio, preferredGenres }
              ...
              <div className="flex items-center gap-6">
                ...
                <div className="flex-1 space-y-2">
                  ...
                  <p className="text-xs text-muted-foreground">
                    프로필에 표시될 이미지 URL을 입력하세요. (Phase 2에서 직접 업로드 지원 예정)
                  </p>
                </div>
